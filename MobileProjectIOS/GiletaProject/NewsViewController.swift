...
final class NewsViewController: UIViewController {
    ...
    init(newsRepository: NewsRepositoryProtocol = CachedNewsRepository()) {
    ...
    override func viewDidLoad() {
        ...
        loadCachedNewsAndRefresh()
        newsRepository.purgeExpiredCache(completion: nil)
    ...
    private func loadCachedNewsAndRefresh() {
        newsRepository.loadCachedNews { [weak self] payload in
            DispatchQueue.main.async {
                guard let self else { return }

                if let payload, payload.articles.isEmpty == false {
                    self.apply(payload)
                    self.loadingIndicator.stopAnimating()
                }

                self.fetchNews(showBlockingLoader: self.articles.isEmpty)
            }
        }
    }
    ...
    private func fetchNews(showBlockingLoader: Bool) {
        ...
        newsRepository.refreshLatestNews { [weak self] result in
            DispatchQueue.main.async {
                guard let self else { return }
    ...
    private func apply(_ payload: NewsPayload) {
        articles = payload.articles
        stateLabel.text = articles.isEmpty ? "Новостей пока нет" : nil
        retryButton.isHidden = true
        tableView.isHidden = articles.isEmpty
        tableView.reloadData()
        navigationItem.prompt = sourceDescription(for: payload.source)
    }

    private func handleRefreshFailure(_ error: NewsServiceError) {
        if articles.isEmpty {
            tableView.isHidden = true
            stateLabel.text = "Не удалось загрузить новости.\n\(error.localizedDescription)"
            retryButton.isHidden = false
            navigationItem.prompt = nil
        } else {
            navigationItem.prompt = "Нет сети. Показаны сохранённые данные"
            print("Ошибка обновления новостей: \(error.localizedDescription)")
        }
    }

    private func sourceDescription(for source: NewsContentSource) -> String {
        switch source {
        case let .cache(savedAt, isFresh):
            let freshnessText = isFresh ? "свежий кэш" : "старый кэш"
            return "Показан \(freshnessText) • \(cacheDateFormatter.string(from: savedAt))"
        case let .network(receivedAt):
            return "Обновлено из сети • \(cacheDateFormatter.string(from: receivedAt))"
        }
    }
